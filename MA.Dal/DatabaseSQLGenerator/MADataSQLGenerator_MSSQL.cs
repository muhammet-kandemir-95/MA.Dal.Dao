...
{
    public abstract class MADataSQLGenerator_MSSQL : MADataSQLGenerator
    {
        #region Variables
        /// <summary>
        /// AllColumns = "*"
        ...
        public const string AllColumnsString = "*";
        ...
        /// <returns></returns>
        public static string SelectPage(string selectQuery, string orderByCommand, int? startRowIndex, int? endRowIndex, int aliasTableName = 0, bool orderEnable = true)
        {
            ...
            return
                ...
                "FROM (" + selectQuery + @") AS " + MAData.Sql.ObjectName("rt_with_row_next" + aliasTableName) + ") AS " + MAData.Sql.ObjectName("rt_result" + aliasTableName) +
                (whereQueries.Count > 0 ? " WHERE " + string.Join(" AND ", whereQueries.ToArray()) : "") +
                (orderEnable ? " ORDER BY " + MAData.Sql.ObjectName("rt_result" + aliasTableName) + ".ROW_NEXT" : "");
        }
        /// <summary>
        /// Create SELECT query
        /// </summary>
